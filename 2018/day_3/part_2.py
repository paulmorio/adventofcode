...
for area in fabric_claims.keys():
	if len(fabric_claims[area]) >= 2:
		clash_claimers+=fabric_claims[area]

clash_claimers = list(set(clash_claimers))
single_claimers = []
for area in fabric_claims.keys():
	if len(fabric_claims[area]) == 1:
		for claimant in fabric_claims[area]:
			if claimant in clash_claimers:
				pass
			else:
				single_claimers.append(claimant)
		
print(set(single_claimers))